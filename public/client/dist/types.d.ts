 ...
 */
export type MetaPiiField = 'em' | 'ph' | 'fn' | 'ln' | 'ge' | 'db' | 'ct' | 'st' | 'zp' | 'country' | 'external_id';
export type MetaNonPiiField = 'client_ip_address' | 'client_user_agent' | 'fbc' | 'fbp' | 'subscription_id' | 'fb_login_id' | 'lead_id';
export type MetaUserDataField = MetaPiiField | MetaNonPiiField;
export type UserDataAlias = 'email' | 'phone' | 'first_name' | 'last_name' | 'gender' | 'date_of_birth' | 'city' | 'state' | 'zip' | 'zipcode' | 'postal_code' | 'country_code';
...
export interface AdvancedMatchingConfig {
    ...
    formFieldMap: Record<string, MetaPiiField>;
    dataLayerKey: string;
...
export type MetaStandardEvent = 'PageView' | 'ViewContent' | 'AddToCart' | 'AddToWishlist' | 'InitiateCheckout' | 'AddPaymentInfo' | 'Purchase' | 'Lead' | 'CompleteRegistration' | 'Contact' | 'CustomizeProduct' | 'Donate' | 'FindLocation' | 'Schedule' | 'Search' | 'StartTrial' | 'SubmitApplication' | 'Subscribe';
...
export type CaptureSource = 'explicit' | 'identify' | 'form' | 'form_prefill' | 'url' | 'dataLayer' | 'customDataLayer' | 'metatag';
...
export interface DebugInfo {
    ...
    advancedMatching: AdvancedMatchingDiagnostics;
    queueSize: number;
}
export interface DisguisedPayload {
    d: string;
    t: number;
    v: string;
}
